...
template<int E>
static constexpr float EPS_ROUND(float value) {
    fp32_int32 in = {value};
    in.bits -= (0x3fffff>>FP8<E>::M);
    return in.f;
}
...
inline uint8_t from_float(float value) {
    ...
    if (in.f >= FP8<E>::MAX) {
        out.bits |= 0x7E;
    } else if (in.f < EPS_ROUND<E>(FP8<E>::MIN)) { // => 0.
    } else {
...
inline float to_float(const FP8<E>& in) {
    fp32_int32 out = {0};
    out.bits = in.bits & 0x80;       out.bits <<= 24;
    uint32_t _bits = in.bits & 0x7F;    _bits <<= (23-FP8<E>::M);
    out.bits |= _bits;
...
inline uint8_t from_float(float value) {
    ...
    if (in.f >= FQ8<E>::MAX) {
        out.bits |= 0x7F;
    } else if (in.f < EPS_ROUND<E>(FQ8<E>::MIN)) { // => 0.
    } else {
        in.bits -= 0x3F800000;
        uint32_t eps = (0x3fffff>>FQ8<E>::M) + ((in.bits >> (23-FQ8<E>::M)) & 0x1);
        in.bits += eps;
        out.bits |= (in.bits >> (23-FQ8<E>::M)) & 0x7F;
    }
    return out.bits;
}
...
#endif
template<int E>
inline float to_float(const FQ8<E>& in) {
    fp32_int32 out = {0};
    out.bits = in.bits & 0x80;        out.bits <<= 24;
    uint32_t _bits = in.bits & 0x7F;     _bits <<= (23-FQ8<E>::M);
    if (_bits) _bits += 0x3F800000;
    out.bits |= _bits;
    return out.f;
}
} // namespace fp8
...
template<int E>
static inline void conv(const float* x, FQ8<E>* y, int64_t size) {
#ifdef GGML_USE_OPENMP_SIMD
    #pragma omp simd
#endif
    for (int64_t i=0; i<size; i++) {
        y[i].bits = fq8::from_float<E>(x[i]);
    }
}
...
template<int E>
static inline float dot(const FQ8<E>* x, const float* y, int64_t size) {
    float z = 0;
#ifdef GGML_USE_OPENMP_SIMD
    #pragma omp simd reduction(+:z)
#endif
    for (int64_t i=0; i<size; i++) {
        z += fq8::to_float(x[i])*y[i];
    }
    return z;
}
...
template <int E, int QK>
static inline void conv(const float* x, bloc_fq8<E, QK>* y, int64_t size) {
    const auto qk_size = size / QK;
    for (int64_t q=0; q<qk_size; ++q) {
        float m = 0;
#ifdef GGML_USE_OPENMP_SIMD
        ...
        const float D = FQ8<E>::MAX/m;
        y[q].d = m/FQ8<E>::MAX;
#ifdef GGML_USE_OPENMP_SIMD
        #pragma omp simd
#endif
        for (int64_t i=0; i<QK; i++) {
            y[q].qs[i].bits = fq8::from_float<E>(x[q*QK+i]*D);
        }
    }
}
...
template <int E, int QK>
static inline float dot(const bloc_fq8<E, QK>* x, const float* y, int64_t size) {
    float z = 0;
    const auto qk_size = size / QK;
    for (int64_t q=0; q<qk_size; ++q) {
        float z0 = 0;
        for (int64_t i=0; i<QK; i++) {
            z0 += fq8::to_float(x[q].qs[i])*y[q*QK+i];
        }
        z += (x[q]).d * z0;
    }
    return z;
...
static float dot_reg(const bloc_fp8<E, QK>* x, const _Y* y, int64_t size) {
...
template <int VECT_SIZE, int NB_REG, int E, int QK, typename _Y>
static float dot_reg(const bloc_fq8<E, QK>* x, const _Y* y, int64_t size) {
    static_assert(QK%(VECT_SIZE*NB_REG)==0, "size not supported");
    using fq8_t = FQ8<E>;
    ...
    for (int64_t q=0; q<qk_size; ++q) {
        ...
        for (int64_t i=0; i<QK; i+=VECT_SIZE*NB_REG) {
            for(int64_t r=0; r<NB_REG; ++r) {
                ...
                for(int64_t v=0; v<VECT_SIZE; ++v) { sign_16bits[v] <<= 8; }
                for(int64_t v=0; v<VECT_SIZE; ++v) { mantice_16bits[v] <<= (7-fq8_t::M); }

                for(int64_t v=0; v<VECT_SIZE; ++v) { x_bf16[v]  = sign_16bits[v] | mantice_16bits[v]; }
                for(int64_t v=0; v<VECT_SIZE; ++v) { if(mantice_16bits[v]) { x_bf16[v] += 0x3F80; } }

                for(int64_t v=0; v<VECT_SIZE; ++v) { ux[v].bits = x_bf16[v]; }
                for(int64_t v=0; v<VECT_SIZE; ++v) { ux[v].bits <<= 16; }

                for(int64_t v=0; v<VECT_SIZE; ++v) { X[v] = ux[v].f; }
                for(int64_t v=0; v<VECT_SIZE; ++v) { Y[v] = (float)y[q*QK+i+r*VECT_SIZE+v]; }
                // TODO: AVX512-BF16 => dot2 ?
                for(int64_t v=0; v<VECT_SIZE; ++v) { Z0[r][v] += X[v]*Y[v]; }
            }
...
void ggml_vec_dot_e4m3_q(int n, float * s, size_t bs, const block_e4m3_q * vx, size_t bx, const float * vy, size_t by, int nrc) {
    ...
    GGML_UNUSED(by);
    GGML_UNUSED(bs);
#if defined(__AVX512F__) // 32x float32x16_t
    *s = dot_reg<16,4>(reinterpret_cast<const bloc_fp8<4, FP8_K>*>(vx), vy, n);
#elif defined(__AVX__) || defined(__AVX2__)  // 16x float32x8_t
    *s = dot_reg<8,4>(reinterpret_cast<const bloc_fp8<4, FP8_K>*>(vx), vy, n);
#elif defined(__ARM_NEON) // 32x float32x4_t
    *s = dot_reg<4,4>(reinterpret_cast<const bloc_fp8<4, FP8_K>*>(vx), vy, n);
// #elif defined(__ARM_FEATURE_FP16_VECTOR_ARITHMETIC)  // 32x float16x8_t
#else
    *s = dot(reinterpret_cast<const bloc_fp8<4, FP8_K>*>(vx), vy, n);
#endif
}

void ggml_vec_dot_e3m4_q(int n, float * s, size_t bs, const block_e3m4_q * vx, size_t bx, const float * vy, size_t by, int nrc) {
    //ggml_vec_dot_e4m3_q(n,s,bs,(const block_e4m3_q *)vx,bx,vy,by,nrc);
    //return;
    assert(nrc == 1);
    GGML_UNUSED(nrc);
    GGML_UNUSED(bx);
    ...
    GGML_UNUSED(bs);
#if defined(__AVX512F__) // 32x float32x16_t
    *s = dot_reg<16,4>(reinterpret_cast<const bloc_fp8<3, FP8_K>*>(vx), vy, n);
#elif defined(__AVX__) || defined(__AVX2__)  // 16x float32x8_t
    *s = dot_reg<8,4>(reinterpret_cast<const bloc_fp8<3, FP8_K>*>(vx), vy, n);
#elif defined(__ARM_NEON) // 32x float32x4_t
    *s = dot_reg<4,4>(reinterpret_cast<const bloc_fp8<3, FP8_K>*>(vx), vy, n);
// #elif defined(__ARM_FEATURE_FP16_VECTOR_ARITHMETIC)  // 32xfloat16x8_t
#else
    *s = dot(reinterpret_cast<const bloc_fp8<3, FP8_K>*>(vx), vy, n);
#endif
}
...
void ggml_vec_dot_fq8_4(int n, float * s, size_t bs, const block_fq8_4 * vx, size_t bx, const float * vy, size_t by, int nrc) {
    ...
    GGML_UNUSED(bs);
#if defined(__AVX512F__) // 32x float32x16_t
    *s = dot_reg<16,4>(reinterpret_cast<const bloc_fq8<4, FQ8_K>*>(vx), vy, n);
#elif defined(__AVX__) || defined(__AVX2__)  // 16x float32x8_t
    *s = dot_reg<8,4>(reinterpret_cast<const bloc_fq8<4, FQ8_K>*>(vx), vy, n);
#elif defined(__ARM_NEON) // 32x float32x4_t
    *s = dot_reg<4,4>(reinterpret_cast<const bloc_fq8<4, FQ8_K>*>(vx), vy, n);
// #elif defined(__ARM_FEATURE_FP16_VECTOR_ARITHMETIC)  // 32x float16x8_t
#else
    *s = dot(reinterpret_cast<const bloc_fq8<4, FQ8_K>*>(vx), vy, n);
#endif
}
...
void ggml_vec_dot_fq8_3(int n, float * s, size_t bs, const block_fq8_3 * vx, size_t bx, const float * vy, size_t by, int nrc) {
    ...
    GGML_UNUSED(bs);
#if defined(__AVX512F__) // 32x float32x16_t
    *s = dot_reg<16,4>(reinterpret_cast<const bloc_fq8<3, FQ8_K>*>(vx), vy, n);
#elif defined(__AVX__) || defined(__AVX2__)  // 16xfloat32x8_t
    *s = dot_reg<8,4>(reinterpret_cast<const bloc_fq8<3, FQ8_K>*>(vx), vy, n);
#elif defined(__ARM_NEON) // 32x float32x4_t
    *s = dot_reg<4,4>(reinterpret_cast<const bloc_fq8<3, FQ8_K>*>(vx), vy, n);
// #elif defined(__ARM_FEATURE_FP16_VECTOR_ARITHMETIC)  // 32xfloat16x8_t
#else
    *s = dot(reinterpret_cast<const bloc_fq8<3, FQ8_K>*>(vx), vy, n);
#endif
}
