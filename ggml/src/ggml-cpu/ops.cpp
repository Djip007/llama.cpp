...
static void ggml_compute_forward_gelu_bf16(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ggml_vec_gelu_bf16(nc,
                (ggml_bf16_t *) ((char *) dst->data  + i1*( dst->nb[1])),
                (ggml_bf16_t *) ((char *) src0->data + i1*(src0->nb[1])));

#ifndef NDEBUG
        for (int k = 0; k < nc; k++) {
            const ggml_bf16_t x = ((ggml_bf16_t *) ((char *) dst->data + i1*( dst->nb[1])))[k];
            const float v = GGML_CPU_BF16_TO_FP32(x);
            GGML_UNUSED(v);
            assert(!isnan(v));
            assert(!isinf(v));
        }
...
static void ggml_compute_forward_clamp_bf16(
    const ggml_compute_params * params,
    ggml_tensor * dst) {
    ...
    for (int j = ith; j < n; j += nth) {
        ggml_bf16_t * dst_ptr  = (ggml_bf16_t *) ((char *)  dst->data + j*nb1);
        ggml_bf16_t * src0_ptr = (ggml_bf16_t *) ((char *) src0->data + j*nb01);

        for (int i = 0; i < nc; i++) {
            float v = GGML_BF16_TO_FP32(src0_ptr[i]);
            dst_ptr[i] = GGML_FP32_TO_BF16(MAX(MIN(v, max), min));
        }
    }
}
...
void ggml_compute_forward_clamp(
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_BF16:
            {
                ggml_compute_forward_clamp_bf16(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
