...
#                   pragma unrool KV
            ...
            if ((J0+j1*N0) < N) {
                for (hipint_t id = tid; id<(K1*K0*N0)/NV; id+= NB_THREAD) {
                    ...
                    //B_cache[j0 + k0*N0 + j1*N0*K0*K1 + j2*N0*K0*K1*N1] = B_trans_out[k0][j0];
    ...
    )
    {
        ...
        //const bool LOAD_A = tid_<(NB_THREAD/2);
        //const bool LOAD_B = LOAD_A?false:true;
        //const hipint_t tid = tid_%(NB_THREAD/2);
        const hipint_t tid = tid_;
        ...
        // chargement de A
        //if (LOAD_A) {
            //for (hipint_t id = tid; id<ML*K0*M1*M2*K1; id+=(NB_THREAD/2)) {
        ...
        // chargement de B
        //if (LOAD_B) {
            //for (hipint_t id = tid; id<NL*K0*K1*N1*N2; id+=(NB_THREAD/2)) {
