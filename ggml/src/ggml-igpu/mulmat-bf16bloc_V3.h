...
#                       pragma unrool K0
    ...
    // - gemv:
    template< int TYPE,
    hipint_t K2, hipint_t M2,
    hipint_t K1, hipint_t M1,
    hipint_t K0, hipint_t M0=16,hipint_t KV=2,
            ...
            hipint_t M, hipint_t K)
    {
        ...
        float32_t   c_frag[M2] = {};  // M0*M1 thread...
        ...
        for (hipint_t k2=0; k2<K2; ++k2) {
            ...
            bfloat16x2_t a_frag[K0];
            bfloat16x2_t b_frag[K0];

            for (hipint_t k1=0; k1<K1; ++k1) {
                // chargement de b_frag (le meme pour chaque i1 & copier sur chaque lignes)
#               pragma unrool K0
                for (hipint_t k0=0; k0<K0; k0++) {
                    b_frag[k0] = B_frag[k2][k1][k0];
                }

                for (hipint_t i2=0; i2<M2; ++i2) {
#                   pragma unrool K0
                    for (hipint_t k0=0; k0<K0; k0++) {
                        a_frag[k0] =  A[i2][i1][k0+k1*K0+k2*K0*K1][i0];
                    }

                    for (hipint_t k0=0; k0<K0; k0++) {
                        c_frag[i2] = __builtin_amdgcn_fdot2_f32_bf16(a_frag[k0], b_frag[k0], c_frag[i2], false);
                    }
                }
            }
        }
        ...
        for (hipint_t i2 = 0; i2<M2 ; ++i2) {
            if constexpr(TYPE == DEBUT) {
                c[i0+i1*M0+i2*M0*M1+i3*M0*M1*M2] = c_frag[i2];
            }
    ...
    //----------------------------------
    // - gemv:
    template<std::size_t _K0, std::size_t _M1, std::size_t _K1, std::size_t _M2, std::size_t _K2>
    void sgemv_wmma(const bfloat16_t* A, const float32_t* B, float32_t* C, std::size_t M, std::size_t K) {
        constexpr int KV = 2;

        static_assert( _K0%KV == 0);
        static_assert( _K2%(_K1*_K0*KV) == 0);

        constexpr int M0 = 16;
        constexpr int K0 = _K0/KV;

        constexpr int M1 = _M1;
        constexpr int K1 = _K1;

        constexpr int M2 = _M2;
        constexpr int K2 = _K2/(K1*_K0);
        ...
        if (K<=_K2) {
            ...
            hipLaunchKernelGGL(HIP_KERNEL_NAME(wmma_matvec<(DEBUT|FIN),K2,M2,K1,M1,K0>),
                    dim3(M/(M0*M1*M2)), dim3(M0, M1),
                    0, 0,
                    A,B,C, M,K);
            auto res = hipGetLastError();
            if (res != hipSuccess) {
            ...
            for (std::size_t k3=0; k3<K; k3+=_K2) {
                ...
                if (k3 == 0) {
                    // le debut
                    hipLaunchKernelGGL(HIP_KERNEL_NAME(wmma_matvec<DEBUT,K2,M2,K1,M1,K0>),
                            dim3(M/(M0*M1*M2)), dim3(M0, M1),
                            0, 0,
                            &A[k3*M],&B[k3],C, M,K);
                } else if (k3 < K-_K2) {
                    // les milieux
                    hipLaunchKernelGGL(HIP_KERNEL_NAME(wmma_matvec<MILIEUX,K2,M2,K1,M1,K0>),
                            dim3(M/(M0*M1*M2)), dim3(M0, M1),
                            0, 0,
                            &A[k3*M],&B[k3],C, M,K);
                } else {
                    // la fin
                    hipLaunchKernelGGL(HIP_KERNEL_NAME(wmma_matvec<FIN,K2,M2,K1,M1,K0>),
                            dim3(M/(M0*M1*M2)), dim3(M0, M1),
                            0, 0,
                            &A[k3*M],&B[k3],C, M,K);
                }
...
//#define BENCH_COMPUTE
        ...
        if (N == 0) {
            return true;
        } else if (N==1) { // matvect[4096, 1, 4096]<2,8,1,1,1,1,1024>
            if (M%(8*BLOC_M0)==0) { // M=128
                sgemv_wmma<2,8,1,1,BLOC_K1>(A,B,C, M,K);
            } else {
