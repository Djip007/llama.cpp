...
#                       pragma unrool K0
    ...
    __global__ void __launch_bounds__(M0*M1) wmma_matvec(
            const TA* __restrict__ a, const TB* __restrict__ b, TC* __restrict__ c,
            hipint_t M, hipint_t K)
    {
        // only possible/interesting values!
        static_assert(KV==2, "use of __builtin_amdgcn_fdot2_f32_bf16");
        static_assert(M0==16, "use of __builtin_amdgcn_wmma_f32_16x16x16_bf16_w32");

        using A_t = const bfloat16x2_t[M1][K2][M0];
        using B_t = const bfloat16x2_t[N1][K];
        ...
        // les shareds:
        using B_frag_t = bfloat16x2_t[N1][K2];
        using C_frag_t = float32_t[M1][M0];

        __shared__ B_frag_t B_frag;
        __shared__ C_frag_t C_frag;

        // le calcul de C sur K0*K1*K2
        float32_t c_frag[N1] = {};  // M0*M1 thread...
...
#               pragma unrool KV
                for (hipint_t kv=0; kv<KV; ++kv) {
                    B_frag[j1][k2][kv] = conv2bf16(b[k2*2+kv + j1*K]);
                }
            }
        }
        ...
        // calcul
        for (hipint_t k2=0; k2<K2; ++k2) {
            bfloat16x2_t a_frag = A[i1][k2][i0];
            for (hipint_t j1=0; j1<N1; ++j1) {
                bfloat16x2_t b_frag = B_frag[j1][k2];
                c_frag[j1] = __builtin_amdgcn_fdot2_f32_bf16(a_frag, b_frag, c_frag[j1], false);
            }
        }
    ...
    //----------------------------------
    // - gemv:
    template<std::size_t _M1, std::size_t _N1, std::size_t _K2>
    void sgemv_wmma(const bfloat16_t* A, const float32_t* B, float32_t* C, std::size_t M, std::size_t N, std::size_t K) {
        constexpr int KV = 2;
        ...
        if (K<=_K2) {
            ...
            hipLaunchKernelGGL(HIP_KERNEL_NAME(wmma_matvec<(DEBUT|FIN),K2,M1,N1>),
            ...
            for (std::size_t k3=0; k3<K; k3+=_K2) {
                ...
                if (k3 == 0) {
                    // le debut
                    hipLaunchKernelGGL(HIP_KERNEL_NAME(wmma_matvec<DEBUT,K2,M1,N1>),
                            dim3(M/(M0*M1)), dim3(M0, M1),
                            0, 0,
                            &A[k3*M],&B[k3],C, M,K);
                } else if (k3 < K-_K2) {
                    // les milieux
                    hipLaunchKernelGGL(HIP_KERNEL_NAME(wmma_matvec<MILIEUX,K2,M1,N1>),
                            dim3(M/(M0*M1)), dim3(M0, M1),
                            0, 0,
                            &A[k3*M],&B[k3],C, M,K);
                } else {
                    // la fin
                    hipLaunchKernelGGL(HIP_KERNEL_NAME(wmma_matvec<FIN,K2,M1,N1>),
                            dim3(M/(M0*M1)), dim3(M0, M1),
                            0, 0,
                            &A[k3*M],&B[k3],C, M,K);
                }
...
#define SGEMV_8(_N) \
    ...
    if (M%( 8*BLOC_M0)==0) sgemv_wmma< 8,_N,BLOC_K1>(A,B,C, M,N,K);\
    else return false

#define SGEMV_16(_N) \
} else if (N==_N) {\
     if (M%(16*BLOC_M0)==0) sgemv_wmma<16,_N,BLOC_K1>(A,B,C, M,N,K);\
else if (M%( 8*BLOC_M0)==0) sgemv_wmma< 8,_N,BLOC_K1>(A,B,C, M,N,K);\
    else return false

#define SGEMV_32(_N) \
} else if (N==_N) {\
     if (M%(32*BLOC_M0)==0) sgemv_wmma<32,_N,BLOC_K1>(A,B,C, M,N,K);\
else if (M%(16*BLOC_M0)==0) sgemv_wmma<16,_N,BLOC_K1>(A,B,C, M,N,K);\
else if (M%( 8*BLOC_M0)==0) sgemv_wmma< 8,_N,BLOC_K1>(A,B,C, M,N,K);\
else return false
...
//#define BENCH_COMPUTE
        ...
        if (N <= 0) {
            return true;
            SGEMV_8( 1);  // matvect[4096, 1, 4096]<2,8,1,1,1,1,1024>
            SGEMV_8( 2);
            SGEMV_8( 3);
            SGEMV_8( 4);
            SGEMV_8( 5);
            SGEMV_16( 6);  // matmul[4096, 6, 4096]<2,16,6,1,1,1,1024>
            SGEMV_16( 7);
            SGEMV_16( 8);
            SGEMV_16( 9);
            SGEMV_16(10);
            SGEMV_32(11);
            SGEMV_32(12);  // matmul[4096, 12, 4096]<2,32,12,1,1,1,1024>
            SGEMV_32(13);
            SGEMV_32(14);
            SGEMV_32(15);
            SGEMV_32(16);
