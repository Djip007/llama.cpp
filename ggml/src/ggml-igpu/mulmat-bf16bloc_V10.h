...
namespace ggml::backend::igpu {
    ...
    template<> struct Types<float32_t, float32_t, float32_t> {
        ...
        static constexpr int ML = M0;
        static constexpr int NL = N0;
    ...
    template<> struct Types<bfloat16_t, float32_t, float32_t> {
        ...
        static constexpr int N0 = 16;
        static constexpr int M0 = 16;
        static constexpr int ML = M0/4;
        static constexpr int NL = N0/4;
        ...
        using aload_t = bfloat16x8_t;
        using afrag_t = bfloat16x16_t;
        using b_t     = a_t;
        using bvect_t = avect_t;
        using bload_t = aload_t;
        using bfrag_t = afrag_t;
        using c_t     = float32_t;
    ...
    __global__ void __launch_bounds__(32) pack(
        ...
        constexpr hipint_t NV = 4;
        using conv_t  = type_t<TB_OUT>::t;
        using conv2_t = type_t<TB_OUT>::x2;
        using conv8_t = type_t<TB_OUT>::x8;
        
        constexpr hipint_t BV = 4; // 4x float32x2 
        using in_t  = type_t<TB_IN>::t;
        using in8_t = type_t<TB_IN>::x8;
        ...
        // cache dans LDS: 
        // TODO: voir a reduire la taille necessaire K1 <= 128
        __shared__ char data[sizeof(conv2_t)*K1*K0*N0]; //(N0+NV)];
        using cache_in_t  = conv2_t[K1*K0][N0   ];// + NV];
        using cache_out_t = conv8_t[K1*K0][N0/NV];// +  1];
        auto&       B_trans_in  = *reinterpret_cast<cache_in_t*>  (&data[0]);
        const auto& B_trans_out = *reinterpret_cast<cache_out_t*> (&data[0]);

        // output
        //using B_cache_t = conv8_t[N/(N0*N1]][N1][K1*K0][N0/4];
        using B_cache_t = conv8_t[N1][K1*K0][N0/NV];
        auto* __restrict__ B_cache = (B_cache_t* __restrict__)b_cache;
        // input
        const auto* __restrict__ B = reinterpret_cast<const in8_t*> (b);
        ...
        for (hipint_t j1=0; j1<N1; ++j1) {
            ...
            for (hipint_t id = tid; id<(K1*K0*Nx/BV); id+= NB_THREAD) {
                const hipint_t k0 = id%(K0*K1/BV);
                const hipint_t j0 = id/(K0*K1/BV);
                const hipint_t j  = (j0+j1*N0+j2*N0*N1);
                
                in8_t Bx8 = B[k0 + j*K/(KV*BV)]; // 8x32 bits...
#               pragma unrool BV
                for (hipint_t ki=0; ki<BV; ++ki) {
#                   pragma unrool KV
                    for (hipint_t kv=0; kv<KV; ++kv) {
                        B_trans_in[k0*BV+ki][j0][kv] = conv2t<conv_t>(Bx8[kv+ki*KV]);
                    }
                }
            }
            if constexpr (NB_THREAD>32) __syncthreads();
            // store: ecriture par pack de 8 bf16 => 128 bits
            if ((J0+j1*N0) < N) {
                for (hipint_t id = tid; id<(K1*K0*N0)/NV; id+= NB_THREAD) {
                    //                         j2       j1   k0     j0
                    // B_Cache: bfloat16x8_t[N/(N0*N1]][N1][K1*K0][N0/4];
                    const hipint_t j0 = id%(N0/NV);
                    const hipint_t k0 = id/(N0/NV);
                    B_cache[j2][j1][k0][j0] = B_trans_out[k0][j0];
                }
            }
            if constexpr (NB_THREAD>32) __syncthreads();
        }
    ...
    )
    {
        ...
        using a_load_t = t::aload_t;
        using b_vect_t = t::bvect_t;
        using b_load_t = t::bload_t;
        ...
        constexpr hipint_t ML = t::ML;
        constexpr hipint_t NL = t::NL;
        
        using A_t = const a_load_t[M2][M1][K2*K1*K0][ML];
        using B_t = const b_load_t[N2][N1][K2*K1*K0][NL];

        //                                [ NB_THREAD   ]
        using C_t = c_comp_t[N1][M1][M0/2][N2][M2][2][N0];

        const hipint_t tid_ = threadIdx.x + 32 * threadIdx.y + 32*M2*threadIdx.z;
        constexpr hipint_t NB_THREAD = 32*M2*N2/2;

        // la 1er moitier des thread charge A, l'autre moitier charge B...
        const bool LOAD_A  = tid_<NB_THREAD;
        const bool LOAD_B  = !LOAD_A;
        const hipint_t tid = tid_%NB_THREAD;
        ...
        // les shareds:
        using A_frag_in_t   = a_load_t[K1][M2][M1][K0][ML];
        using A_frag_out_t  = a_vect_t[K1][M2][M1][K0][M0];
        using B_frag_in_t   = b_load_t[K1][N2][N1][K0][NL];
        using B_frag_out_t  = b_vect_t[K1][N2][N1][K0][N0];
        using C_frag_t  = c_comp_t[N0][M2*M1*M0+1];
        
        static_assert(sizeof(A_frag_in_t) == sizeof(A_frag_out_t));
        static_assert(sizeof(B_frag_in_t) == sizeof(B_frag_out_t));

        // TODO: tester avec FlipFlop ...
        __shared__ char data[HIP_MAX((sizeof(A_frag_in_t) + sizeof(B_frag_in_t))*2, sizeof(C_frag_t))];
        auto*       A_frag_in  = reinterpret_cast<A_frag_in_t*>  (&data[0]);
        const auto* A_frag_out = reinterpret_cast<A_frag_out_t*> (&data[0]);

        auto*       B_frag_in  = reinterpret_cast<B_frag_in_t*>  (&data[sizeof(A_frag_in_t)*2]);
        const auto* B_frag_out = reinterpret_cast<B_frag_out_t*> (&data[sizeof(A_frag_in_t)*2]);
        ...
        // chargement de A
        if (LOAD_A) {
            for (hipint_t id = tid; id<ML*K0*M1*M2*K1; id+=NB_THREAD) {
                const hipint_t il =  id%ML;
                const hipint_t k0 = (id/ML)%K0;
                const hipint_t k1 = (id/(ML*K0))%K1;
                const hipint_t i1 = (id/(ML*K0*K1))%M1;
                const hipint_t i2 =  id/(ML*K0*K1*M1);
                A_frag_in[0][k1][i2][i1][k0][il] = A[i2][i1][k0+k1*K0][il];
            }
        }
        // chargement de B
        if (LOAD_B) {
            for (hipint_t id = tid; id<NL*K0*K1*N1*N2; id+=NB_THREAD) {
                const hipint_t jl =  id%NL;
                const hipint_t k0 = (id/NL)%K0;
                const hipint_t k1 = (id/(NL*K0))%K1;
                const hipint_t j1 = (id/(NL*K0*K1))%N1;
                const hipint_t j2 =  id/(NL*K0*K1*N1);
                if((j2*N0*N1 + j3*N0*N1*N2) < N) {
                    B_frag_in[0][k1][j2][j1][k0][jl] = B[j2][j1][k0+k1*K0][jl];
                }
            }
        }
            ...
            if (k2+1<K2) {
                ...
                if (LOAD_A) {
                    for (hipint_t id = tid; id<ML*K0*M1*M2*K1; id+=NB_THREAD) {
                        ...
                        const hipint_t i1 = (id/(ML*K0*K1))%M1;
                        const hipint_t i2 =  id/(ML*K0*K1*M1);
                        A_frag_in[load][k1][i2][i1][k0][il] = A[i2][i1][k0+k1*K0+(k2+1)*K0*K1][il];
                    }
                }
            }
...
#                           pragma unrool K0
                            for (int k0=0; k0<K0; ++k0) {
                                b_frag_in[j1][k0] = B_frag_out[use][k1][j2][j1][k0][j0];
                            }
                        }
                    }
...
#                       pragma unrool K0
                        for (int k0=0; k0<K0; ++k0) {
                            a_frag_in[k0] = A_frag_out[use][k1][i2][i1][k0][i0];
                        }
                        for (hipint_t j1=0; j1<N1; ++j1) {
            ...
            if (k2+1<K2) {
                ...
                if (LOAD_B) {
                    for (hipint_t id = tid; id<NL*K0*K1*N1*N2; id+=NB_THREAD) {
                        ...
                        const hipint_t j2 =  id/(NL*K0*K1*N1);
                        if((j2*N0*N1 + j3*N0*N1*N2) < N) {
                            B_frag_in[load][k1][j2][j1][k0][jl] = B[j2][j1][k0+k1*K0+(k2+1)*K0*K1][jl];
                        }
                    }
                }
            }
            ...
            hipint_t M, hipint_t K)
    {
        ...
        // __shared__ ? A_frag_in / A_frag_out;
        ...
        // TODO: vectorisé ce chargement (facile)
        for (hipint_t j1=0; j1<N1; ++j1) {
...
#               pragma unrool KV
        ...
        // calcul
        for (hipint_t k2=0; k2<K2; ++k2) {
            // TODO: @ vectoriser => utiliser la LDS pour A aussi...
            //  => plus compliqué ? => bloc sur k2 ...
...
#                       pragma omp simd
    ...
    bool _compute(const TA* A, const TB* B, TC* C,
        ...
        if (N<32) {  // ici N>10
            // M=256
            if (M%(2*8*BLOC_M0)==0) { sgemm_wmma<2,1,2,8,1,BLOC_K1>(A,B,C, M,N,K); return true; }
            // M=128
            if (M%(2*4*BLOC_M0)==0) { sgemm_wmma<2,1,2,4,1,BLOC_K1>(A,B,C, M,N,K); return true; }
            return false;
        } else if (N<=48) {  // matmul[4096, 48, 4096]<2,1,2,8,4,1024>  48 OK..
            // - llama-8B: <=48
            // - Nemo-12B: < 192
            //  => TODO: tuner plus finement (fct (M,K,N) ?)
            // M=256
        ...
        } else { // matmul[4096, 64, 4096]<2,1,2,4,4,1024>
            // M=128
            if (M%(2*4*BLOC_M0)==0) { sgemm_wmma<2,1,2,4,4,BLOC_K1>(A,B,C, M,N,K); return true; }
        }
        // oups...
